...
| `InvalidOrderStatusTransitionError` | 400         | `InvalidOrderStatusTransitionError` | "Transição de status inválida para o pedido" |
...
class InvalidOrderStatusTransitionError extends DomainError {
  // Carrega o status atual e o status de destino inválido
}
```
...
```json
{
  "error": "ValidationError",
  "message": "Dados inválidos",
  "details": [{ "field": "items", "message": "items must contain at least 1 item" }]
}
```
...
- 2026-03-23: Criação inicial do documento com contrato de erros, exemplos e instruções de implementação.
- 2026-03-29: Adicionado InvalidOrderStatusTransitionError na tabela de erros mapeados (T2.5).
